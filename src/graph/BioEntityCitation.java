...
public class BioEntityCitation {
	...
	public static void main(String[] args) throws Exception {
			...
			while(s.hasNext()) {
				...
				for(int i=0; i<abs_list.size(); i++)
					for(int j=0; j<list.size(); j++) {
						...
						int idx1 = entityIndex.get(a);
						int idx2 = entityIndex.get(b);
		...
		PrintWriter printer = new PrintWriter(new FileWriter("data/graph/bio-entity-citation-edges.txt"));
		...
		printer.close();
		printer = new PrintWriter(new FileWriter("data/graph/bio-entity-citation-nodes.txt"));
